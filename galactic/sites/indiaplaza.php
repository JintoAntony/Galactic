...
function flipkart($searchQuery)
{
   
    $searchQuery=urlencode($searchQuery);
    $searchUrl='http://www.indiaplaza.com/searchproductsall.aspx?sn=all&q='.$searchQuery;
    ...
    if($firstLink!="")  //if number of results is less then there will be no fk-button-blue
    {
        ...
        $newLink='http://www.Indiaplaza.in'.$firstLink->href;
       ...
       foreach($html->find('div.fk-srch-item') as $result) 
       {
            ...
            $item['buyNow']='http://www.Indiaplaza.in'.$buyNowTemp;
            $item['site']="Indiaplaza";
            $items[] = $item;
       }
    }
    else
